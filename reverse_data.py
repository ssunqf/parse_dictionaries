...
def main():
    ...
    p.add_argument('--dictionary_dir', default='/System/Library/AssetsV2/com_apple_MobileAsset_DictionaryServices_dictionaryOSX',
                   help=f"dir to dictionary data. defaults to '/System/Library/AssetsV2/com_apple_MobileAsset_DictionaryServices_dictionaryOSX'")
    p.add_argument('--lookup', nargs='+',
                   default=['vital', 'house', 'cozen'],
                   help='words to lookup')
    p.add_argument('--output_dir', default='./lookup',
                   help='where to save the words.')

    flags = p.parse_args()
    for dirpath, dirs, files in os.walk(flags.dictionary_dir):
        if dirpath.endswith('/Contents/Resources') and 'Body.data' in files:
            name = dirpath.split('/')[-3].replace(' ', '_')
            output_path = os.path.join(flags.output_dir, name + '.xml')
            save_definitions(os.path.join(dirpath, 'Body.data'),
                             flags.lookup,
                             output_path)
...
def _split(input_bytes, verbose) -> Tuple[List[Tuple[str, str]],
    ...
    while True:
        ...
        if not entry_text.startswith('<d:entry') or not entry_text.endswith('</d:entry>'):
            printv(f'ENTRY: {entry_text} \n REM: {input_bytes}')
            break
...
class Entry:
    ...
    def get_xml_tree(self):
        ...
        return etree.fromstring(content, parser=XMLParser(huge_tree=True))
